...
// =============================================================================
// -----------------------------------------------------------------------------
BMinusLCutFlowTracker::BMinusLCutFlowTracker()
{
  initCutFlowHists();
}
...
// -----------------------------------------------------------------------------
void BMinusLCutFlowTracker::fillHist(int channel, int bin, float weight)
{
...
void BMinusLCutFlowTracker::printToScreen()
{
  ...
  for (unsigned int cut_it = 0; cut_it != BMINUSL_CUT_N; ++cut_it) {
    printLine(cut_it);
  }
  std::cout << m_single_line;
}
...
// -----------------------------------------------------------------------------
void BMinusLCutFlowTracker::printLine(int cut_it)
{
  ...
  if (cut_name == "BREAK") {
    ...
    std::cout << "= "  << std::left  << std::setw(m_label_field) << std::setprecision(m_precision) << cut_name
              << " = " << std::right << std::setw(m_weight_field) << std::setprecision(m_precision) << weight_all
...
// -----------------------------------------------------------------------------
void BMinusLCutFlowTracker::initBinList()
{
  m_bin_list.clear();

  for (int cut_it = 0; cut_it != BMINUSL_CUT_N; ++cut_it) {
    m_bin_list.push_back(BMINUSL_CUT_STRINGS[cut_it]);
  }
}

// -----------------------------------------------------------------------------
void BMinusLCutFlowTracker::initCutFlowHists()
{
  initBinList();
