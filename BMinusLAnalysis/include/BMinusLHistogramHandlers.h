...
namespace PennSusyFrame
{
  ...
  // = jet multiplicity
  // =============================================================================
  class JetMultiplicityHists : public HistogramHandler
  {
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    public:
      JetMultiplicityHists(std::string name_tag = "");
      virtual ~JetMultiplicityHists();

      virtual void FillSpecial( const PennSusyFrame::Event& event
                              , const std::vector<PennSusyFrame::Jet*>* good_jets
                              , const std::vector<PennSusyFrame::Jet*>* b_jets
                              , float weight
                              );
      virtual void write(TDirectory*);
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    private:
      // number of all jets, lights jets, and b-jets
      std::vector<TH1F*> m_h_num_jets;
      std::vector<TH1F*> m_h_num_b_jets;
      std::vector<TH1F*> m_h_num_light_jets;
  };
}
