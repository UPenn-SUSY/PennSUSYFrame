#include "EwkAnalysis/include/IsrCutFlowTracker.h"
...
// =============================================================================
// -----------------------------------------------------------------------------
IsrCutFlowTracker::IsrCutFlowTracker()
{
  initCutFlowHists();
}
...
// -----------------------------------------------------------------------------
void IsrCutFlowTracker::fillHist(int channel, int bin, float weight)
{
  m_cutflow.at(channel)->Fill(bin, weight);
}

// -----------------------------------------------------------------------------
void IsrCutFlowTracker::printToScreen()
{
  // set line widths etc.
  m_line_width   = 100;
  m_label_field  = 30;
  ...
  for (unsigned int cut_it = 0; cut_it != ISR_CUT_N; ++cut_it) {
    printLine(cut_it);
  }
  std::cout << m_single_line;
}
...
// -----------------------------------------------------------------------------
void IsrCutFlowTracker::printLine(int cut_it)
{
  std::string cut_name = ISR_CUT_STRINGS[cut_it];
...
// -----------------------------------------------------------------------------
void IsrCutFlowTracker::initBinList()
{
  m_bin_list.clear();

  for (int cut_it = 0; cut_it != ISR_CUT_N; ++cut_it) {
    m_bin_list.push_back(ISR_CUT_STRINGS[cut_it]);
  }
}

// -----------------------------------------------------------------------------
void IsrCutFlowTracker::initCutFlowHists()
{
  initBinList();
